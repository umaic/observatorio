...
class Controller_Reports extends Controller
{
    ...
    public function action_getData()
    {
        ...
        if (count($ids) == 0) {
            ...
            $queryPlaces = DB::select('posts.form_id', 'ab.name')->from('posts')
                ->join(array('forms', 'form'), 'LEFT')
                ->on('posts.form_id', '=', 'form.id')
                ->join(array('post_point', 'pp'), 'LEFT')
                ->on('posts.id', '=', 'pp.post_id')
                ->join(array('admin_boundaries', 'ab'), 'LEFT')
                ->on('pp.admin_boundary_id', '=', 'ab.id')
        ...
        $resultPlaces = $queryPlaces->execute()->as_array();
        ...
        if (count($result) == 0) {
            ...
            foreach ($resultPlaces as $k => $v) {
                $name = strtolower(str_replace(" ", "_", $v['name']));
                if(!isset($totals_places[$name])){
                    $totals_places[$name] = [];
                    $totals_places[$name]['name'] = $v['name'];
                    $totals_places[$name]['forms'] = [];
                }
                foreach ($forms as $key => $value) {
                    $totals_places[$name]['forms'][$value['id']] = 0;
                }
            }
            foreach ($resultPlaces as $rp) {
                $name = strtolower(str_replace(" ", "_", $rp['name']));
                foreach ($forms as $key => $value) {
                    if ($rp['form_id'] == $value['id']) {
                        $totals_places[$name]['forms'][$value['id']] += 1;
                    }
                }
            }
