    ...
    SetPostRepository
{
    ...
    public function getEntity(Array $data = null)
    {
        ...
        if (!empty($data['id'])) {
            $data += [
                ...
                'actors' => $this->getActorsForPost($data['id']),
                'sources' => $this->getSourcesForPost($data['id']),
     ...
     */
    private function getActorsForPost($id)
    {
        $result = DB::select('actor_id')->from('post_tag_actor')
            ->where('post_id', '=', $id)
            ->execute($this->db);
     ...
     */
    private function getSourcesForPost($id)
    {
        $result = DB::select('source_id')->from('post_source_detail')
            ->where('post_id', '=', $id)
            ->execute($this->db);
     ...
     * @return array      source ids for post
     */
    private function getSourcesListForPost($id)
    {
        $result = DB::select('*')->from('post_source_detail')
            ->where('post_id', '=', $id)
            ->execute($this->db);
        return $result->as_array();
    }
    ...
    public function create(Entity $entity)
    {
        ...
        // Handle legacy post.actors attribute
        if ($entity->actors) {
            // Find first actor attribute
            list($attr_id, $attr_key) = $this->getFirstActorAttr($entity->form_id);
    ...
    public function getFirstActorAttr($form_id)
    {
        $result = DB::select('form_attributes.id', 'form_attributes.key')
            ->from('form_attributes')
            ->join('form_stages', 'INNER')->on('form_stages.id', '=', 'form_attributes.form_stage_id')
